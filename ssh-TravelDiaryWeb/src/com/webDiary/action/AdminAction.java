...
import com.webDiary.service.impl.AdminServiceImpl;
...
public class AdminAction {
	...
	public void check() throws IOException {
		...
		List<Admin> aList = adminService.login(admin.getUserName(), admin.getPasswd());// 调用数据库查询方法，验证用户名、密码
		boolean flag = true;
		HttpSession session = ServletActionContext.getRequest().getSession();
		 session.setAttribute("admin", aList.get(0));
		String checkcode1 = (String) session.getAttribute("checkcode");
		if (!aList.isEmpty()) {
			if (aList.get(0).getState() != 1) {
				flag = true;
			} else {
				session.setAttribute("admin", aList.get(0));
				flag = false;
		...
		if (!checkcode1.equalsIgnoreCase(getCheckcode())) {
			String flag1 = "error";
			data = JSON.toJSONString(flag1);
		}
	...
	public void enableOrDiscontinuation() {
		System.out.println("discontinuation():" + admin.getId());
		String data1 = JSON.toJSONString(adminService.enableOrdiscontinuation(admin.getId(),
				((admin.getState() == 1 || admin.getState() == 2) ? admin.getState() : 1)));
	...
	public String all() {
		...
		Admin adm = new Admin();
		List<Admin> mList = adminService.findAll();
		for (Admin admin : mList) {
			if (admin.getId() == id) {
				adm = admin;
				break;
			}
		...
		HttpSession session = ServletActionContext.getRequest().getSession();
		session.setAttribute("admin2", adm);
